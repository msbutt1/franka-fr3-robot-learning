#!/usr/bin/env bash
# Usage: from fr3_real/, run: bash training/prepare_fr3_v3_dataset.sh
# Audit, stage, convert, sample, and compute custom stats for the FR3 v3 DROID dataset.
...
MERGED_RAW="${MERGED_RAW:-${FR3}/fr3_real_recordings_droid_v3_staged}"
...
"${PYTHON}" "${SCRIPT_DIR}/../model-validation/audit_fr3_new_demos.py" \
  ...
  --verify-video-decode \
  --output "${FR3}/fr3_new_demos_audit_v3.json"
...
uv run --no-sync python "${SCRIPT_DIR}/train_fr3_phase_filtered.py" \
  ...
  --exp-name validate_fr3_v3_alignment \
