#!/usr/bin/env python3
# Usage: from fr3_real/, run: python franka-tests/pick_and_place.py --help
...
sys.path.insert(0, str(FR3_ROOT))
sys.path.insert(0, str(FR3_ROOT / "data-pipeline"))
...
parser.add_argument("--points", type=str, default=str(CONFIGS_DIR / "probed_points.json"))
...
parser.add_argument("--filter_failed_cells_json", action="store_true",
                    help="After a tracked failure, atomically remove only that printed_cell from --cells_json.")
...
parser.add_argument("--place_from_basket_only", action="store_true",
                    help="Pick the cube from the basket, place it on the selected cell, retreat home, then exit without recording or updating trackers.")
...
parser.add_argument("--settle_velocity_threshold", type=float, default=0.01,
                    help="Require measured max joint velocity below this value (rad/s) at motion boundaries; 0 disables.")
parser.add_argument("--settle_velocity_timeout", type=float, default=1.0,
                    help="Seconds to wait for the measured joint-velocity settle condition before aborting the move.")
...
if args.filter_failed_cells_json and (not args.cells_json or not args.status_tracker):
    raise SystemExit("--filter_failed_cells_json requires both --cells_json and --status_tracker.")
if args.place_from_basket_only and not args.recycle_from_basket:
    raise SystemExit("--place_from_basket_only requires --recycle_from_basket.")
...
def remove_failed_cell_from_json(source: dict | None) -> None:
    if not args.filter_failed_cells_json or source is None:
        return
    cells_path = Path(args.cells_json)
    data = json.loads(cells_path.read_text())
    printed_cell = int(source["printed_cell"])
    old_cells = data.get("cells", [])
    new_cells = [
        cell for cell in old_cells
        if int(cell.get("printed_cell", -1)) != printed_cell
    ]
    if len(new_cells) == len(old_cells):
        print(f"  [filter] printed_cell={printed_cell} is already absent from {cells_path}.")
        return
    data["cells"] = new_cells
    temp_path = cells_path.with_suffix(cells_path.suffix + ".tmp")
    temp_path.write_text(json.dumps(data, indent=2) + "\n")
    temp_path.replace(cells_path)
    print(f"  [filter] removed failed printed_cell={printed_cell} from {cells_path} "
          f"({len(old_cells)} -> {len(new_cells)} cells).")
...
def update_status_tracker(source: dict | None, status: str, note: str = "") -> None:
    ...
    if status in {"FAIL", "SETUP_FAIL", "RECORDING_FAIL"}:
        remove_failed_cell_from_json(source)
...
motion = MotionPlanner(
    ...
    settle_velocity_timeout=args.settle_velocity_timeout,
)


def open_gripper() -> None:
    if recorder is not None:
        recorder.set_gripper_command(gripper.max_width, "gripper_open")
    gripper.open(args.open_speed)


def grasp_gripper() -> bool:
    if recorder is not None:
        recorder.set_gripper_command(args.grasp_width, "gripper_grasp")
...
def retreat_home_after_release() -> None:
    try:
        motion.travel_to(home_xyz, home_xyz[2], "retreat home")
    except Exception as exc:
        message = str(exc).lower()
        home_error = float(np.linalg.norm(motion.current_xyz() - home_xyz))
        terminal_finish_warning = "motion finished commanded" in message or "still moving" in message
        if not terminal_finish_warning or home_error > 0.003 or gripper.is_grasped:
            raise
        print(f"  [home] controller reported a terminal finish warning, but measured home error is "
              f"{home_error * 1000:.1f} mm and gripper is open; accepting completed retreat.")
        if robot.has_errors:
            recovered = robot.recover_from_errors()
            if not recovered or robot.has_errors:
                raise
        time.sleep(max(0.30, args.settle_time))
...
def stop_after_setup_failure(source: dict | None, run_index: int, context: str, exc: Exception) -> None:
    print(f"  [setup-blocked] {context}: {exc}")
    print("  [setup-blocked] no recorded episode was active; tracker and cells JSON are unchanged.")
    write_resume_file(source, run_index, "setup_blocked_motion")
    if recorder is not None and recorder.active:
    ...
    try:
        ...
        raise SystemExit("[robot] stopped after setup failure. Hand-guide home, then retry this cell.")
...
for k, cell_xyz in enumerate(cells):
    ...
    if args.recycle_from_basket:
        ...
        try:
            ...
            if not basket_picked:
                print("  [setup-blocked] could not pick cube from basket; the target cell was not attempted.")
                print("  [setup-blocked] tracker and cells JSON are unchanged; restart after placing the cube in the basket.")
                open_gripper()
                motion.travel_to(home_xyz, home_xyz[2], "retreat home")
                write_resume_file(source, k, "setup_blocked_basket_pickup")
                raise SystemExit("[robot] stopped because basket pickup failed; resume remains on this cell.")
            ...
            if args.place_from_basket_only:
                print("  [done] cube placed on selected cell; exiting without recording or tracker updates.")
                break
